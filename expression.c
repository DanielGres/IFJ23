...
bool Expression(struct bst_tok_node *seed, bst_node **root, char *EOE) {
...
bool Viktor_gucci(int *condition, bst_node **root, int isCallLater, struct bst_tok_node *seed) {
    ...
    stack *my_stack = malloc(sizeof(stack));
    stack_init(my_stack);
    stack_push(my_stack, DOLLAR);  // Dolar na spodok staku

    stack_of_T *token_stack = malloc(sizeof(stack_of_T));       // IDK
    stack_of_node *node_stack = malloc(sizeof(stack_of_node));  // IDK
    ...
    // Podmienka je uz useless, da sa zjednodusit, podmienka je na nacitavanie kym neni EOE
    while ((!((current_symbol == BRACKETE) || (current_symbol == DOLLAR))) || (!((my_stack->top->symbol == ENTERPRISE) && (item_symblcmp(my_stack->top->next, DOLLAR))))) {
        // VEZMI PRVY TERMINAL, AK JE FIRST E Tak ber next znak
        ...
        switch (table_symbol) {
                // SHIFT
            case S:
                // AK NIE JE PRVY ZNAK E shiftni hned na vrch
                if (my_stack->top->symbol != ENTERPRISE) {
                    stack_push(my_stack, SHIFT);
                    stack_push(my_stack, current_symbol);
                }
                // AK JE DAVAS SHIFT PRED NEHO A NOVY SYMBOL ZA NEHO
                else {
